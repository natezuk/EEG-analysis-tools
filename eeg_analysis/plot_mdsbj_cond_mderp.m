function plot_mdsbj_cond_mderp(ctrl_erps,asd_erps,dly,cond,chan_to_plot,chan_lbls,exp_condition,...
...
for c = 1:length(unq_cond)
    ...
    [ctrlmdERP(:,:,c),ctrldistr] = spline_median(ctrl_cond,spline_ds,3,ones(nctrl,1));
    ctrllqERP(:,:,c) = squeeze(quantile(ctrldistr,stderr_quantiles(1)));
    ctrluqERP(:,:,c) = squeeze(quantile(ctrldistr,stderr_quantiles(2)));
end
% ASD
...
for c = 1:length(unq_cond)
    ...
    % Compute the median across subject median ERPs
    [asdmdERP(:,:,c),asddistr] = spline_median(asd_cond,spline_ds,3,ones(nasd,1));
    asdlqERP(:,:,c) = squeeze(quantile(asddistr,stderr_quantiles(1)));
    asduqERP(:,:,c) = squeeze(quantile(asddistr,stderr_quantiles(2)));
end
...
if ~isempty(fl_suffix)
    img_fl = sprintf('fig/AllSbj_%s_mdsbjbycond_indivchans_%s.png',exp_condition,fl_suffix);
else
    img_fl = sprintf('fig/AllSbj_%s_mdsbjbycond_indivchans.png',exp_condition);
end
saveas(gcf,img_fl);