...
%     use the same lbl, and each column is plotted as a different color
...
tot_span = 0.8; % span of all dots and median plots for a single label
...
if iscell(yvals),
    if ~iscell(lbl), error('Labels must be contained in a cell array');
        ...
        if sum(dim_check)>1,
            error('Not all cells of labels and y-values have the same dimensions');
        end
    end
else
...
unqlbls = unique(lbl{1}); % get all unique labels
    % assumes all cells use the same labels
